...
public class AdminService {
    ...
    public List<TalkVo> getHistoryTalks(long loginId, Long pageNum, Long pageSize) {
        Page<History> histories = historyMapper.selectPage(new Page<>(pageNum, pageSize), new QueryWrapper<History>().eq("user_id", loginId).eq("type", "talk"));
        List<TalkVo> talkVos = new ArrayList<>();
        for (History record : histories.getRecords()) {
            TalkVo talk = talkService.getSingleTalk(record.getContentId());
            talkVos.add(talk);
        }
        return talkVos;
    }
